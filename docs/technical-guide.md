...
file loads with the session default (1500 µm × 50 µm). **Apply W/L to All
Files** writes the workspace values to every file; editing a file's own
geometry row overrides it for that file alone. The `Geometry` column records
which applies: `default`, `global`, or `manual`.
