...
ParamEx is a Cargo workspace with two crates. `paramex-core` computes; `paramex-gui`
presents. The boundary between them is the single most important rule in the
codebase, and most of the repository's tests exist to keep it in place.
...
|---|---|
| `shared` | Product-agnostic primitives: numerics, NumPy-parity helpers (`numpy_compat`), grid ingest for CSV/TSV/TXT/XLS/XLSX, file identity, and the CSV writer. Nothing here knows about transfer curves or TLM. |
...
| NumPy parity | `tests/shared/numpy_compat/` against `tests/reference/numpy_compat/` | exact tie-breaks, rounding, NaN handling |
